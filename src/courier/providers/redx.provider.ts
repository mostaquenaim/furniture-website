...
export class RedxProvider implements CourierProviderInterface {
  ...
  constructor(
    ...
    this.baseUrl = this.configService.get(
      'REDX_API_URL',
      'https://api-hermes.redx.com',
    );
    this.clientId = this.configService.get('REDX_CLIENT_ID');
    this.clientSecret = this.configService.get('REDX_CLIENT_SECRET');
    this.username = this.configService.get('REDX_USERNAME');
    this.password = this.configService.get('REDX_PASSWORD');
  }
  ...
  private async getRedxAccessToken(): Promise<string> {
    const now = Date.now();
    ...
    try {
      ...
      this.logger.debug('Redx access token refreshed successfully');

      return this.accessToken;
    } catch (err) {
      this.logger.error('Token Error:', err.response?.data || err.message);
      throw new Error(
        `Failed to get Redx access token: ${err.response?.data?.message || err.message}`,
      );
    }
  }
    ...
    try {
      ...
      const response = await firstValueFrom(
        this.httpService.post(
          ...
          {
            store_id: parseInt(data.store_id), // Redx expects number
          ...
          {
            headers: {
              Authorization: `Bearer ${token}`, // Redx uses Bearer token
      ...
      throw new Error(
        `Redx: ${error.response?.data?.message || error.message}`,
      );
    }
  }
  ...
  async trackShipment(trackingId: string): Promise<any> {
    try {
      ...
      this.logger.error('Redx tracking error:', error);
  ...
  async cancelShipment(shipmentId: string): Promise<any> {
    try {
      ...
      this.logger.error('Redx cancel error:', error);
      throw new Error(`Failed to cancel shipment: ${error.message}`);
    }
  }

  calculateRate(data: any): any {
    // Redx might have a rate calculation endpoint
    return {
